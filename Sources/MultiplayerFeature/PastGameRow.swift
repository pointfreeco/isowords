...
public struct PastGame: ReducerProtocol {
  ...
  public func reduce(into state: inout State, action: Action) -> EffectTask<Action> {
    ...
    case let .matchResponse(.success(match)):
      return .send(.delegate(.openMatch(match)), animation: .default)
    ...
    case .rematchButtonTapped:
      ...
      return .run { [matchId = state.matchId] send in
        await send(
          .rematchResponse(TaskResult { try await self.gameCenter.turnBasedMatch.rematch(matchId) })
        )
      }
    ...
    case .tappedRow:
      return .run { [matchId = state.matchId] send in
        await send(
          .matchResponse(TaskResult { try await self.gameCenter.turnBasedMatch.load(matchId) })
        )
      }
    }
  }
