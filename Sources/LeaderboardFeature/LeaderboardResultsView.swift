    ...
    VStack(alignment: .leading, spacing: 16) {
      HStack {
        self.title
          ...
          .redacted(reason: self.store.isLoading ? .placeholder : [])

        Spacer()

        Button {
          self.store.send(.tappedTimeScopeLabel, animation: .default)
        } label: {
          HStack {
            self.timeScopeLabel
            ...
            Image(systemName: "chevron.down")
              .font(.system(size: 10))
              .rotationEffect(
                .degrees(self.store.isTimeScopeMenuVisible ? -180 : 0)
              )
          }
          .padding(.vertical, 8)
      ...
      VStack(alignment: .leading, spacing: 0) {
        if self.isFilterable {
          HStack(spacing: .grid(4)) {
            ForEach(GameMode.allCases) { gameMode in
              Button {
                self.store.send(.gameModeButtonTapped(gameMode))
              } label: {
                Text(gameMode.title)
                  .adaptiveFont(.matterMedium, size: 12)
                  .opacity(self.store.gameMode == gameMode ? 1 : 0.4)
              }
        ...
        ScrollView {
          ...
          Group {
            ForEach(
              self.store.resultEnvelope?.contiguousResults ?? [], id: \.id
            ) { result in
              Button {
                self.store.send(.tappedRow(id: result.id))
              } label: {
                ResultRow(color: self.color, result: result)
              }
            }

            if let result = self.store.resultEnvelope?.nonContiguousResult {
            ...
            if self.store.nonDisplayedResultsCount > 0 {
              VStack(spacing: .grid(5)) {
                Image(systemName: "ellipsis")
                  .opacity(0.4)
                Text("and \(self.store.nonDisplayedResultsCount) more!")
              }
      ...
      .overlay(
        self.store.isTimeScopeMenuVisible
          ? Color.black.opacity(0.4)
            .onTapGesture {
              self.store.send(.dismissTimeScopeMenu, animation: .default)
            }
          : nil
      )
